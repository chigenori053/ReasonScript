...
When declared, the entry is compiled first and the remaining `src/**/*.rsn`
files form the rest of the module graph. Manifests without `[source]` retain
legacy recursive discovery and do not require `src/main.rsn`.
