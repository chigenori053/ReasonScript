...
def compile_package_sources(sources: list[tuple[str, Path]]) -> PipelineResult:
    """Compile a package as one closed module graph instead of per-file islands."""
...
def _package_program(sources: list[tuple[str, Path]]) -> ProgramNode:
    ...
    try:
        ...
        if len(packages) > 1:
            raise PipelineError("SyntaxError", "PV-2 package declaration conflicts across source files")
        package = next((unit.package for unit in units if unit.package is not None), None)
        program = ProgramNode(tuple(module for unit in units for module in unit.modules), package)
