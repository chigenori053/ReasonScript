...
use reasonscript_runtime_real::core::{
    transition::TransitionOp, ReasonUnit, State, Transition, TypeChecker,
};
...
fn test_type_checker_valid_transition() {
    ...
    assert!(
        TypeChecker::check_graph(&graph).is_ok(),
        "Concept -> IsA -> Concept should be valid"
    );
}
...
fn test_type_checker_invalid_transition() {
    ...
    let state_a = State::new(
        StateType::Action,
        ReasonUnit::new("Run", UnitType::Symbolic, array![1.0]),
    );
    let state_b = State::new(
        StateType::Object,
        ReasonUnit::new("Car", UnitType::Symbolic, array![1.0]),
    );
