...
fn sf_a_002_fragment_decomposition_and_type_failure() {
    ...
    let id_animal_s = graph.add_state(State::new(
        StateType::Attribute,
        ReasonUnit::new("Animal_F", UnitType::Symbolic, array![1.0]),
    ));
    let id_animal = graph.add_node(Node::new(id_animal_s));
    let id_canine_s = graph.add_state(State::new(
        StateType::Attribute,
        ReasonUnit::new("Canine_F", UnitType::Symbolic, array![1.0]),
    ));
    let id_canine = graph.add_node(Node::new(id_canine_s));
    let id_domestic_s = graph.add_state(State::new(
        StateType::Attribute,
        ReasonUnit::new("Domestic_F", UnitType::Symbolic, array![1.0]),
    ));
    ...
    assert!(
        result.is_err(),
        "SF-A-002: Inference should fail for Attribute fragments"
    );
...
fn sf_b_001_fragment_inference_isolation() {
    ...
    let id_canine_s = graph.add_state(State::new(
        StateType::Attribute,
        ReasonUnit::new("Canine", UnitType::Symbolic, array![1.0]),
    ));
    let id_canine = graph.add_node(Node::new(id_canine_s));
    let id_mammal_s = graph.add_state(State::new(
        StateType::Attribute,
    ...
    let canine_is_animal = graph
        .edges
        .iter()
        .any(|e| e.source == id_canine && e.target == id_animal);
    assert!(
        !canine_is_animal,
        "SF-B-001: Fragments should not form inference closure"
    );
}
...
fn sf_c_001_dog_reconstruction() {
    ...
    assert_eq!(
        reconstructed_dog.vector, expected_dog_vec,
        "SF-C-001: Reconstruction vector mismatch"
    );
    assert!(
        reconstructed_dog.label.contains("Dog") || reconstructed_dog.label.contains("Animal"),
        "Label should reflect components"
    );
}
...
fn sf_d_001_trace_comparison() {
    ...
    // RU Case
    let id_dog_s = graph_ru.add_state(State::new(
        StateType::Concept,
        ReasonUnit::new("Dog", UnitType::Symbolic, array![1.0]),
    ));
    let id_dog = graph_ru.add_node(Node::new(id_dog_s));
    let id_mammal_s = graph_ru.add_state(State::new(
        StateType::Concept,
        ReasonUnit::new("Mammal", UnitType::Symbolic, array![1.0]),
    ));
    ...
    let id_c_s = graph_sf.add_state(State::new(
        StateType::Attribute,
        ReasonUnit::new("Canine", UnitType::Symbolic, array![1.0]),
    ));
    let id_c = graph_sf.add_node(Node::new(id_c_s));
    let id_m_s = graph_sf.add_state(State::new(
        StateType::Attribute,
        ReasonUnit::new("Mammal_F", UnitType::Symbolic, array![1.0]),
    ));
