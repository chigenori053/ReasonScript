...
use reasonscript_runtime_real::core::{SemanticContext, ActivationState, DynamicsContext};
use reasonscript_runtime_real::graph::{ReasonGraph, Node, Edge};
use reasonscript_runtime_real::executor::{Executor, ExecutionContext, dynamics::Dynamics};
use ndarray::array;
...
fn test_sd_007_depth_limit() {
    ...
    let id_sa = graph.add_state(State::new(StateType::Event, ReasonUnit::new("A", UnitType::Symbolic, array![1.0])));
    let id_a = graph.add_node(Node::new(id_sa));
    let id_sb = graph.add_state(State::new(StateType::Event, ReasonUnit::new("B", UnitType::Symbolic, array![1.0])));
    let id_b = graph.add_node(Node::new(id_sb));
    let id_sc = graph.add_state(State::new(StateType::Event, ReasonUnit::new("C", UnitType::Symbolic, array![1.0])));
    let id_c = graph.add_node(Node::new(id_sc));
    let id_sd = graph.add_state(State::new(StateType::Event, ReasonUnit::new("D", UnitType::Symbolic, array![1.0])));
