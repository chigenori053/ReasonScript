...
fn if_a_001_identity_emergence_dog() {
    ...
    // 1. Fragments
    let animal_f = ReasonUnit::new("Animal", UnitType::Real, Array1::from_vec(vec![1.0, 0.0, 0.0, 0.0]));
    let canine_f = ReasonUnit::new("Canine", UnitType::Real, Array1::from_vec(vec![0.0, 1.0, 0.0, 0.0]));
    let domestic_f = ReasonUnit::new("Domestic", UnitType::Real, Array1::from_vec(vec![0.0, 0.0, 1.0, 0.0]));
    let pet_f = ReasonUnit::new("Pet", UnitType::Real, Array1::from_vec(vec![0.0, 0.0, 0.0, 1.0]));
    ...
    let id_mammal_s = graph.add_state(State::new(StateType::Concept, ReasonUnit::new("Mammal", UnitType::Real, array![1.0, 1.0, 0.0, 0.0])));
    let id_mammal = graph.add_node(Node::new(id_mammal_s));
    let t = Transition::new(TransitionType::Deduction, TransitionOp::Addition(ReasonUnit::zero(4, UnitType::Real)));
...
fn if_b_001_identity_stability_repeated_activation() {
    ...
    assert!(matches!(final_context.trace[0], TraceEvent::IdentityStabilization(_)));
}
...
fn if_d_001_identity_evolution_puppy_to_dog() {
    ...
    let id_puppy_s = graph.add_state(State::new(StateType::Event, ReasonUnit::new("Puppy", UnitType::Real, array![1.0, 1.0, 1.0, 0.5])));
    let id_puppy = graph.add_node(Node::new(id_puppy_s));
    
    let id_dog_s = graph.add_state(State::new(StateType::Event, ReasonUnit::new("Dog", UnitType::Real, array![1.0, 1.0, 1.0, 1.0])));
    let id_dog = graph.add_node(Node::new(id_dog_s));

    // Evolution as a transition
    let t = Transition::new(TransitionType::Search, TransitionOp::Addition(ReasonUnit::new("Growth", UnitType::Real, array![0.0, 0.0, 0.0, 0.5])));
    ...
    exec_context.record_trace(TraceEvent::IdentityTransition { from: id_puppy, to: id_dog });
    
    assert!(exec_context.trace.iter().any(|e| matches!(e, TraceEvent::IdentityTransition { .. })));
}
