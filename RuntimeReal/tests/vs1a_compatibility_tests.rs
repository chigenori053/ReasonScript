...
use reasonscript_runtime_real::core::{ActivationState, SemanticContext};
...
fn test_b_002_real_valued_taxonomic_closure() {
    ...
    assert!(
        closure_exists,
        "Real-valued taxonomic closure should generate new edges"
    );
}
...
fn test_b_003_topology_retention() {
    ...
    let id_ss1 = g_sym.add_state(State::new(
        StateType::Concept,
        ReasonUnit::new("S1", UnitType::Symbolic, array![1.0]),
    ));
    let id_s1 = g_sym.add_node(Node::new(id_ss1));
    let id_ss2 = g_sym.add_state(State::new(
        StateType::Concept,
        ReasonUnit::new("S2", UnitType::Symbolic, array![1.0]),
    ));
    let id_s2 = g_sym.add_node(Node::new(id_ss2));
    let id_ss3 = g_sym.add_state(State::new(
        StateType::Concept,
        ReasonUnit::new("S3", UnitType::Symbolic, array![1.0]),
    ));
    let id_s3 = g_sym.add_node(Node::new(id_ss3));

    let t_sym = Transition::new(
        TransitionType::Deduction,
    ...
    let id_rs1 = g_real.add_state(State::new(
        StateType::Concept,
        create_real_ru("R1", [1.0; 16]),
    ));
    let id_r1 = g_real.add_node(Node::new(id_rs1));
    let id_rs2 = g_real.add_state(State::new(
        StateType::Concept,
        create_real_ru("R2", [0.9; 16]),
    ));
    let id_r2 = g_real.add_node(Node::new(id_rs2));
    let id_rs3 = g_real.add_state(State::new(
        StateType::Concept,
        create_real_ru("R3", [0.8; 16]),
    ));
    let id_r3 = g_real.add_node(Node::new(id_rs3));

    let t_real = Transition::new(
        TransitionType::Deduction,
        TransitionOp::Addition(ReasonUnit::zero(16, UnitType::Real)),
    );
