...
fn ru_a_002_sub_reasonunit_decomposition() {
    ...
    // D -> o -> g
    let id_d_s = graph.add_state(State::new(StateType::Concept, ReasonUnit::new("D", UnitType::Symbolic, array![1.0])));
    let id_d = graph.add_node(Node::new(id_d_s));
    let id_o_s = graph.add_state(State::new(StateType::Concept, ReasonUnit::new("o", UnitType::Symbolic, array![1.0])));
    let id_o = graph.add_node(Node::new(id_o_s));
    let id_g_s = graph.add_state(State::new(StateType::Concept, ReasonUnit::new("g", UnitType::Symbolic, array![1.0])));
    ...
    let d_dep_g = graph.edges.iter().any(|e| e.source == id_d && e.target == id_g && e.relation == RelationType::Dependency);
...
fn ru_b_001_object_composition() {
    ...
    let id_dog_s = graph.add_state(State::new(StateType::Object, ReasonUnit::new("Dog", UnitType::Symbolic, array![1.0])));
    let id_dog = graph.add_node(Node::new(id_dog_s));
    
    let id_tail_s = graph.add_state(State::new(StateType::Object, ReasonUnit::new("Tail", UnitType::Symbolic, array![1.0])));
    let id_tail = graph.add_node(Node::new(id_tail_s));
    
    let id_leg_s = graph.add_state(State::new(StateType::Object, ReasonUnit::new("Leg", UnitType::Symbolic, array![1.0])));
    ...
    assert!(exec_context.dynamics.get_state(&id_dog) == reasonscript_runtime_real::core::ActivationState::Visited || 
            exec_context.dynamics.get_state(&id_dog) == reasonscript_runtime_real::core::ActivationState::Active);
}
...
fn ru_b_002_hierarchical_composition() {
    ...
    let id_cell_s = graph.add_state(State::new(StateType::Object, ReasonUnit::new("Cell", UnitType::Symbolic, array![1.0])));
    let id_cell = graph.add_node(Node::new(id_cell_s));
    
    let id_organ_s = graph.add_state(State::new(StateType::Object, ReasonUnit::new("Organ", UnitType::Symbolic, array![1.0])));
    let id_organ = graph.add_node(Node::new(id_organ_s));
    
    let id_body_s = graph.add_state(State::new(StateType::Object, ReasonUnit::new("Body", UnitType::Symbolic, array![1.0])));
    let id_body = graph.add_node(Node::new(id_body_s));

    graph.add_edge(Edge::new(id_cell, id_organ, RelationType::PartOf, t.clone()));
    graph.add_edge(Edge::new(id_organ, id_body, RelationType::PartOf, t.clone()));

    Executor::infer(&mut graph, &mut exec_context, &semantic_context, id_cell).unwrap();

    // Check for Cell PartOf Body
    let cell_part_of_body = graph.edges.iter().any(|e| e.source == id_cell && e.target == id_body && e.relation == RelationType::PartOf);
    assert!(cell_part_of_body, "RU-B-002: Hierarchical PartOf closure failed");
}
...
fn cosine_similarity(v1: &Array1<f64>, v2: &Array1<f64>) -> f64 {
    ...
    if norm1 * norm2 == 0.0 { 0.0 } else { dot / (norm1 * norm2) }
}
...
fn ru_c_001_semantic_distance() {
    ...
    assert!(sim_dog_mammal > sim_dog_car, "RU-C-001: Semantic distance violation");
}
...
fn ru_d_002_trace_validation() {
    ...
    let id_a_s = graph.add_state(State::new(StateType::Concept, ReasonUnit::new("A", UnitType::Symbolic, array![1.0])));
    let id_a = graph.add_node(Node::new(id_a_s));
    let id_b_s = graph.add_state(State::new(StateType::Concept, ReasonUnit::new("B", UnitType::Symbolic, array![1.0])));
