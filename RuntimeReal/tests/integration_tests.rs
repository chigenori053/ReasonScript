...
use reasonscript_runtime_real::core::{SemanticConstraint, SemanticContext, SemanticRule};
...
fn test_full_runtime_integration() {
    ...
    let state_dog = State::new(
        StateType::Concept,
        ReasonUnit::new("Dog", UnitType::Symbolic, array![1.0]),
    );
    let state_mammal = State::new(
        StateType::Concept,
    ...
    assert!(
        result.is_ok(),
        "Inference step should succeed for a well-typed, semantically valid graph"
    );
    assert!(result.unwrap());
    assert!(exec_context.timestamp >= 1);
    assert!(
        exec_context.active_nodes.contains(&id_node_mammal),
        "Target node should be activated"
    );
}
