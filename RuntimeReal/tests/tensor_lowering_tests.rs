...
use reasonscript_runtime_real::ir::{lowering::Lowering, tensor_ir::TensorOp, GraphIR};
...
fn test_tensor_ir_lowering() {
    ...
    let state_a = State::new(
        StateType::Concept,
        ReasonUnit::new("A", UnitType::Vector, vec_a.clone()),
    );
    ...
    let state_b = State::new(
        StateType::Concept,
        ReasonUnit::new("B", UnitType::Vector, vec_b.clone()),
    );
