...
fn run_request(request: &serde_json::Value) -> ExitCode {
    ...
    let executable_reason_unit_name = request
        .pointer("/context/executable_reason_units")
        .and_then(serde_json::Value::as_str)
        .unwrap_or("off");
    let Some(executable_reason_unit_mode) = ExecutableMode::parse(executable_reason_unit_name)
    else {
        return fail_request(
            request_id,
            "RTH-PROTO-004",
    ...
    let reasoning_state_name = request
        .pointer("/context/reasoning_state")
        .and_then(serde_json::Value::as_str)
        .unwrap_or("off");
    let Some(reasoning_state_mode) = ReasoningStateMode::parse(reasoning_state_name) else {
        return fail_request(
            request_id,
            "RTH-PROTO-004",
            "reasoning_state must be off or lightweight",
        );
    };
    if reasoning_state_mode.enabled() && executable_reason_unit_mode != ExecutableMode::Full {
        return fail_request(
            request_id,
            "RUS-005",
    ...
    match vm.run_calculations(&program) {
        Ok(calculations) => {
            ...
            let reasoning_state = vm.reasoning_state_trace();
            ...
            let reason_unit_trace = vm.reason_unit_trace();
