...
use reasonunit_phase1_test::math_reason::{
    Poly, MathState, is_valid_transition,
};
use reasonunit_phase1_test::semantic_space::{
    build_english_space, build_cross_language_space,
    SemanticVector,
};
...
fn decompose_semantic_vector(vec: &SemanticVector) -> Vec<SemanticVector> {
    ...
    vec.label.chars().map(|c| {
        SemanticVector::new(&c.to_string(), [0.0; 16])
    }).collect()
}
...
fn ru_2_equation_transition_decomposition() {
    ...
    let s0 = MathState::Equation { lhs: Poly::new(vec![2.0, 1.0]), rhs: Poly::new(vec![5.0]) };
    ...
    assert!(!valid, "Fragmented transitions should lose equivalence context");
}
...
fn ru_4_cross_language_decomposition() {
    ...
    assert!(avg_fragment_dist > 0.9, "Fragments should lose cross-language alignment");
}
...
fn ru_5_reasoning_chain_decomposition() {
    ...
    let pcr = if pcs_before > 0.0 { pcs_after / pcs_before } else { 0.0 };
